...
function addTimezones() {
  ...
  for([k, v] of Object.entries(worldTimeZones)) {
    ...
    optionFrom.value=k;
    optionTo.value=k;
    if (optionFrom.value == userSavedTimezoneFrom) {
      optionFrom.selected = true;
    }
    if (optionTo.value == userSavedTimezoneTo) {
...
chrome.storage.sync.get(["timezoneFrom", "timezoneTo"], (zone) => {
  userSavedTimezoneFrom = zone.timezoneFrom;
  userSavedTimezoneTo = zone.timezoneTo;
  addTimezones();
});
...
function saveTimezone() {
  ...
  chrome.storage.sync.set({
    timezoneFrom: selectFrom.value,
    timezoneTo: selectTo.value,
  });
}

document.getElementById("saveTimezone").addEventListener("click", saveTimezone);